...
- [Hiearchical Clustering](#hiearchical-clustering)
  - [Results of Hiearchical
    Clustering](#results-of-hiearchical-clustering)
...
The first 3 PCs add up to 72.64% of the original variance. Hence 3 PCs
are required to describe at least 70% of the original variance in the
data.

> Q6. How many principal components (PCs) are required to describe at
> least 90% of the original variance in the data?

The first 7 PCs add up to 91.01% of the original varaiance. Hence 7 PCs
are required to describe at least 90% of the original variance in the
data.
...
The plot is extremely difficult to understand. The labels are
overlapping with each other and is unreadable. Also the data overlaps
with each other and is not visible for us to interpret.
...
The plots slightly differ in how the points are spread. On the PC2
versus PC3 scale, it seems that the points on the PC2 scale are spread
out more while the points on the PC3 scale are slightly more packed.
...
is -0.2608538. There are no other features with larger contriutions than
this one.
...
distance matrix (with the `dist()` function). Then cluster with
...
You can also use `cutree()` function with a argument `k=4` rather than
`h = height`
...
There are different definitions of quality that may vary between
clustering.
...
Using “ward.D2” gives the favored result in my opinion. The first few
clusters are easy to see and there is a clear difference in Height among
them as opposed to methods like “single”.
...
There are 24 False Positives (FP), 179 True Positives (TP), 333 True
Negatives (TN), 33 False Negatives (FN) in this newly created hclust
model with 2 clusters of “M” and “B” diagnoses.
...
There are more groups in the original cluster with few individuals in
the 2 separate groups. There seems to be relatively less False Positives
in group 1, but slightly more false negatives in group 2.
