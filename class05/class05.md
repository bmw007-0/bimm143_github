...
p <- ggplot(cars) + aes(x=speed,  y=dist) + geom_point() + 
  ...
  labs(title = "Stopping distance of old cars", 
       subtitle = "Data from the `cars` object", x="Speed (mph)",
       y="Distance (ft)")
```
...
ggplot(genes) + aes(Condition1, Condition2, col=State) + geom_point() + 
  ...
  labs(title="Gene Expression Changes upon GLP-1 Inhibitor",
    x="Control (no drug)", y="Drug Treatment") + theme_bw()
```
...
ggplot(input) + aes(x=gdpPercap, y=lifeExp, col=continent, size=pop) + 
